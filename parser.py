...
def parse_file(fname, edges, polygons, transform, screen, color):
    ...
    while c < len(lines):
        ...
        if line == 'push':
            ...
            tempMat = []
            add_sphere(tempMat, float(args[0]), float(args[1]), float(args[2]),
            ...
            tempMat = []
            add_torus(tempMat, float(args[0]), float(args[1]), float(args[2]),
            ...
            #print 'BOX\t' + str(args)
            tempMat = []
            add_box(tempMat, float(args[0]), float(args[1]), float(args[2]),
            ...
            tempMat = []
            add_circle(tempMat, float(args[0]), float(args[1]), float(args[2]),
                       float(args[3]), step)
            matrix_mult(stack[-1], tempMat)
            ...
            tempMat = []
            add_curve(tempMat, float(args[0]), float(args[1]), float(args[2]),
                      float(args[3]), float(args[4]), float(args[5]),
            ...
            #print 'LINE\t' + str(args)
            tempMat = []
            add_edge(tempMat, float(args[0]), float(args[1]), float(args[2]),
                     float(args[3]), float(args[4]), float(args[5]))
            matrix_mult(stack[-1], tempMat)
            draw_lines(tempMat, screen, color)
